...
export const ASSIGNMENT_WINDOW_MS = 2 * 60 * 1000; // 2 minutos, como pediste
...
export async function findEligibleLiquidators(
  ...
  const { data: liquidators, error } = await supabase
    .from("liquidators")
    .select(
      `id, status, max_concurrent_ops, payment_methods, spread_bps,
  ...
  for (const l of liquidators ?? []) {
    ...
    eligible.push({
      ...
      spreadBps: l.spread_bps ?? 0,
      availableCollateral: available,
      currentOpenOps: currentOpenOps,
    });
  }

  // Orden: mejor precio primero (spread más bajo), y entre precios iguales,
  // mejor reputación primero. Esto es lo que hace que "spread más bajo te
  // haga aparecer primero" sea cierto, no solo un texto en la UI.
  return eligible.sort((a, b) => {
    if (a.spreadBps !== b.spreadBps) return a.spreadBps - b.spreadBps;
...
/**
 * Asigna una operación recién creada a UN cajero elegible, elegido al azar
 * (no por reputación ni spread — eso solo aplica dentro del pool general).
 * Si no hay ningún cajero elegible en ese momento, la operación entra
 * directo al pool general.
 */
export async function assignRandomProvider(
  supabase: SupabaseClient,
  params: { operationId: string; paymentMethodId: string; amountUsdEquivalent: number }
) {
  const eligible = await findEligibleLiquidators(supabase, {
  ...
  if (eligible.length === 0) {
    await supabase
      .from("operations")
      .update({ pool_stage: "general", assigned_to: null, assignment_expires_at: null })
      .eq("id", params.operationId);
    return { assignedTo: null, poolStage: "general" as const };
  }

  const chosen = eligible[Math.floor(Math.random() * eligible.length)];
  const expiresAt = new Date(Date.now() + ASSIGNMENT_WINDOW_MS).toISOString();

  await supabase
    .from("operations")
    .update({
      pool_stage: "assigned",
      assigned_to: chosen.liquidatorId,
      assignment_expires_at: expiresAt,
    })
    .eq("id", params.operationId);

  return { assignedTo: chosen.liquidatorId, poolStage: "assigned" as const };
}

/**
 * Revisa operaciones cuya ventana de asignación individual (2 min) ya
 * venció sin ser tomadas, y las pasa al pool general visible para todos.
 *
 * Se llama de forma "perezosa" (lazy) cada vez que alguien consulta la
 * cola — funciona sin necesitar un cron real. Para producción, vale la
 * pena moverlo a un cron de verdad (pg_cron en Supabase, o un Edge
 * Function programado) para que la transición sea puntual y no dependa
 * de que alguien abra la app justo en ese momento.
 */
export async function expireStaleAssignments(supabase: SupabaseClient) {
  const nowIso = new Date().toISOString();

  const { data: expired } = await supabase
    .from("operations")
    .select("id")
    .eq("pool_stage", "assigned")
    .eq("status", "pending_match")
